module.exports = {
	...
	inserir: function(logTransferencia, cb){ //Insere as informações passadas pelo servidor
		...
		require('./controller.js').inserir("TBLogTransferencia", logTransferencia, function(codRes){
			cb(codRes);
		});
	},
	...
	alterar: function(logTransferencia, cb){ //Altera as informações passadas por servidor
		...
		require('./controller.js').alterar("TBLogTransferencia", logTransferencia, function(codRes){
			cb(codRes);
		});
	},
	...
	excluir: function(logTransferencia, cb){ //Exclui o registro cujo ID seja igual o ID fornecido pelo servidor
		if(!logTransferencia)
			...
			cb(400);
		require('./controller.js').excluir("TBLogTransferencia", logTransferencia, function(codRes){
			cb(codRes);
		});
	},

	listar: function(cb){ //Lista todos os registros da tabela;
		require('./controller.js').listar("TBLogTransferencia", function(res){
			cb(res);
		});
	},

	buscar: function(argumentos, cb){ //Busca registros na tabela baseado nos argumentos recebidos pelo servidor
		require('./controller.js').buscar("TBLogTransferencia", argumentos, function(res){
			cb(res);
		});		
	}
}