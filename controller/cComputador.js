module.exports = {
	...
	listar: function(cb){ //Lista todos os registros da tabela;
		...
		require('./controller.js').listar("Computador", function(res){
			cb(res);
		}, {campos: "TBComputador.*, p.nome processadorNome, so.nome sistemaNome, i.patrimonio itemPatrimonio, i.ativo ativo, i.id itemId, s.nome setorNome, l.nome localNome, s.id setorId, l.id localId, lt.data dataMovimentacao", 
		joins: [
		...
		where: "lt.atual = 1", orderBy: [{campo: "dataMovimentacao", sentido: "desc"}, {campo: "i.patrimonio", sentido: "asc"}]});
	},
